...
void main()
{
    ...
    // RUNSTACK_COPY
    int op1  = a || b; PrintInteger(op1 == 1);    // LOGICAL_OR
    int op2  = a && b; PrintInteger(op2 == 1);    // LOGICAL_AND
    int op3  = a |  b; PrintInteger(op3 == 3);    // INCLUSIVE_OR
    int op4  = a ^  b; PrintInteger(op4 == 3);    // EXCLUSIVE_OR
    int op5  = a &  b; PrintInteger(op5 == 0);    // BOOLEAN_AND
    int op6  = a == b; PrintInteger(!op6);        // CONDITION_EQUAL
    int op7  = a != b; PrintInteger(op7);         // CONDITION_NOT_EQUAL
    int op8  = a >= b; PrintInteger(!op8);        // CONDITION_GEQ
    int op9  = a >  b; PrintInteger(!op9);        // CONDITION_GT
    int op10 = a <  b; PrintInteger(op10);        // CONDITION_LT
    int op11 = a <= b; PrintInteger(op11);        // CONDITION_LEQ
    int op12 = a << b; PrintInteger(op12 == 4);   // SHIFT_LEFT
    int op13 = a >> b; PrintInteger(op13 == 0);   // SHIFT_RIGHT
    int op14 = a >>> b; PrintInteger(op14 == 0);  // USHIFT_RIGHT
    int op15 = a +  b; PrintInteger(op15 == 3);   // ADD
    int op16 = a -  b; PrintInteger(op16 == -1);  // SUBTRACT
    int op17 = a *  b; PrintInteger(op17 == 2);   // MULTIPLY
    int op18 = a /  b; PrintInteger(op18 == 0);   // DIVIDE
    int op19 = a %  b; PrintInteger(op19 == 1);   // MODULUS
    int op20 = -a; PrintInteger(op20 == -1);      // NEGATION
    int op21 = ~a; PrintInteger(op21 == -2);      // ONES_COMPLEMENT
    int op22 = !a; PrintInteger(op22 == 0);       // BOOLEAN_NOT
    int op23 = ++a; PrintInteger(op23 == 2);      // INCREMENT_BASE
    int op24 = --a; PrintInteger(op24 == 1);      // DECREMENT_BASE

    // TODO: also test float, string, engst, etc.

    if (a) {}                               // JZ
    else { PrintInteger(FALSE); }                 // JMP, NO_OPERATION
    if (a != b) {}                          // JZ
    ...
    PrintInteger(teststruct.y == 5);
    PrintInteger(teststruct.x == 5);

    g = 3; PrintInteger(g == 3);                  // ASSIGNMENT_BASE

    g++; PrintInteger(g == 4);                    // INCREMENT_BASE
    g--; PrintInteger(g == 3);                    // DECREMENT_BASE
    ...
    Random(1);
    // STORE_STATE
    DelayCommand(0.1f, ActionRandomWalk());
    // RET
}
